...
<div class="card mb-4" style="margin:3%;">
  ...
  <div class="row">
          <div class="col-md-6">
          <h3>Visualizar e editar playlist</h3>
          </div>
          <div class="col-md-6">
...
<form action="{{ $pl->id }}" method="post" id="frmUpdate">
  ...
  <div class="form-outline mb-4">
    <input type="text" name="title" class="form-control" placeholder="Titulo" value="{{ $pl->title }}" required/>
   
  </div>
  <!-- Descrição input -->
  <div class="form-outline mb-4">
    <input type="text" name="description" maxlength="200" class="form-control" placeholder="Descrição" value="{{ $pl->description }}" />
  </div>

  <!-- Email input -->
  <div class="form-outline mb-4">
    <input type="text" name="author"  class="form-control" placeholder="Autor" value="{{ $pl->author }}"  required/>
...
<div class="card mb-4" style="margin:1%;">
  <div class="card-body">   
  <div class="row">
          <div class="col-md-6">
          <h4>Conteúdo dessa playlist:</h4>
          </div>
...
@if(count($conteudos) > 0)
    ...
    <small><i class="fa-sharp fa-solid fa-circle"></i></small> {{ $c->title }} <em>(Autor: {{ $c->author }})</em><br>
    @endforeach
@else
    <em>Ainda não existem conteúdos para esta playlist</em>
@endif    
<br>
