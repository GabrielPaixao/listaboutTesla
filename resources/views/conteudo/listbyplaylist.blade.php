...
<div class="row">
          <div class="col-md-6">
          <h3>Conteúdos da playlist <strong>{{ $playlist }}</strong></h3>
          </div>
          <div class="col-md-6">
             <a href="../createCompact/{{ $playlist_id }}" style="float:right"><button type="button" class="btn btn-block btn-success btn-xs"><i class="fa-sharp fa-solid fa-circle-plus"></i> CONTEUDO</button></a>
...
$(document).ready(function () {
    var table = $('#playlists').DataTable({
        ajax: '/api/conteudo/listAjaxByPlayList/{{ $playlist_id }}', //<=== CONSUMINDO A API POR AJAX
        columns: [
            { data: 'id' },
            { data: 'playlist' },
            { data: 'title' },
